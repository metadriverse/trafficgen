...
def draw_real(ax, center, edge, rest, context, path=None, save=False, gt=False):
    ...
    if gt:
        for i in range(context.shape[0]):
            ...
            ax.plot([center[0], vel[0] + center[0]], [center[1], vel[1] + center[1]], '.-r', linewidth=0.04,
                    markersize=0.2, markeredgecolor='none')
...
def draw_seq(center, edge, rest, context, path=None, save=False, gt=False):
    ...
    if gt:
        ...
        for i in range(context.shape[0]):
            ...
            rect = plt.Rectangle([point_x, point_y], W, L, -yaw * 180 / np.pi, edgecolor="black",
                                 facecolor=col, linewidth=0.2)
            if abs(center[0]) < vl and abs(vel[0] + center[0]) < vl and abs(center[1]) < vl and abs(
                    vel[1] + center[1]) < vl:
                ax.plot([center[0], vel[0] + center[0]], [center[1], vel[1] + center[1]], '.-r', linewidth=0.5,
                        markersize=1)
            rect.set_zorder(0)
            ax.add_patch(rect)
...
if __name__ == "__main__":
    ...
    for i in range(0, 10):
        ...
        agent_pos, agent_dir, agent_vel = get_agent_pos_from_vec(vec, data['long_perc'], data['lat_perc'],
                                                                 data['relative_dir'], data['v_value'])
